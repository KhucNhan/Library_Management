...
//        table.refresh();
    ...
    @FXML
    private TextField releaseYearSearch;
    ...
    public void search() {
        ...
        for (Book book : books) {
            boolean matchesTitle = title.isEmpty() || Normalizer.normalize(book.getTitle().toLowerCase(), Normalizer.Form.NFD).replaceAll("[\\p{InCombiningDiacriticalMarks}]", "").contains(title);
            boolean matchesAuthor = author.isEmpty() || Normalizer.normalize(book.getAuthor().toLowerCase(), Normalizer.Form.NFD).replaceAll("[\\p{InCombiningDiacriticalMarks}]", "").contains(author);
            boolean matchesReleaseYear = releaseYear.isEmpty() || book.getReleaseYear().equalsIgnoreCase(releaseYear);

            // Nếu sách phù hợp với tất cả các trường hợp không rỗng, thêm vào danh sách kết quả
